 ...
 * @author Tze-I Yang
 */
public class IntersectedBoundingVolumes implements BoundingVolume {
	private BoundingVolume first;
	private BoundingVolume second;
	
	public IntersectedBoundingVolumes(BoundingVolume first, BoundingVolume second)
	{
		this.first = first;
		this.second = second;
	}
	...
	{
		if(first.collide(other))
			return second.collide(other);
		return false;
	}

	public boolean collide(double x, double y)
	{
		if(first.collide(x, y))
			return second.collide(x, y);
		return false;
	}
